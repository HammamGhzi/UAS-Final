...
const CategoryFormModal = ({
  ...
  useEffect(() => {
    if (open) {
      reset(initialValues ?? emptyCategoryForm);
    }
      ...
      <div className="w-full max-w-md rounded-[24px] bg-white p-6 shadow-[0_24px_60px_rgba(0,0,0,0.2)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#777777]">
                Sesuai field model BatikCategory di backend.
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <label className="block space-y-2">
            <span className="text-sm font-bold text-[#444444]">Nama Kategori</span>
            <input
              {...register("categoryName")}
              autoFocus
              maxLength={100}
              placeholder="mis. Flora, Fauna, Geometris"
              className={`h-[48px] w-full rounded-[16px] border bg-white px-4 text-sm outline-none transition focus:border-[#ff9800] ${
                errors.categoryName ? "border-red-400" : "border-[#c9c9c9]"
              }`}
            />
            {errors.categoryName && (
