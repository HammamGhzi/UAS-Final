...
import {
  emptyProdukEditForm,
  produkEditSchema,
  type ProdukEditFormValues,
} from "@/pages/superAdmin/produkEditStore";
import { useCategories } from "@/pages/superAdmin/useCategories";

type ProdukEditModalProps = {
  open: boolean;
  initialValues?: ProdukEditFormValues;
  isSubmitting?: boolean;
  onClose: () => void;
  onSubmit: (values: ProdukEditFormValues) => void;
};

const ProdukEditModal = ({
  open,
  initialValues,
  isSubmitting = false,
  onClose,
  onSubmit,
}: ProdukEditModalProps) => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<ProdukEditFormValues>({
    resolver: zodResolver(produkEditSchema),
    defaultValues: emptyProdukEditForm,
    mode: "onChange",
  });
  ...
  useEffect(() => {
    if (open) {
      reset(initialValues ?? emptyProdukEditForm);
      setImageError("");
    }
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[28px] bg-white px-8 py-7 shadow-[0_24px_60px_rgba(0,0,0,0.2)]">
        <div className="mb-6 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-[#2f2f2f]">Edit Produk</h2>
              <p className="text-sm text-[#777777]">Ubah data produk sesuai kebutuhan.</p>
            </div>
          </div>
          <button
            type="button"
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <label className="space-y-2 md:col-span-2">
              ...
              <input
                ...
                placeholder="Batik Tulis Harimau"
...
export default ProdukEditModal;